...
import { View, Text, Button, StyleSheet, Image, Modal, FlatList } from 'react-native';
...
//   let players = initialPlayers;
  ...
  const endOneEpisode = () => {
    if (
        ...
        let winnerIndex = Math.floor(Math.random() * 4);
        for (let i = 0; i < players.length; i++) {
            players[winnerIndex].collectedCards.push(players[i].currentPlayedCard);
  ...
  const isEndOneEpisode = players[0].currentPlayedCard != null &&
  players[1].currentPlayedCard != null &&
  players[2].currentPlayedCard != null &&
  players[3].currentPlayedCard != null;
  ...
  const isCurrentPlayerBottom = (currentPlayerIndex === 0); // Assuming player 0 is at the bottom
  ...
  return (
    ...
    <View style={[styles.playedCardContainer, { bottom: 180 }]}>
      ...
      {selectedPlayer && (
        ...
        {/* Conditional Button */}
        {isEndOneEpisode ? 
            <Button title="End this Round" onPress={endOneEpisode} /> : 
        (isCurrentPlayerBottom ? (
            <Button title="Play Selected Card" onPress={handlePlaySelectedCard} />
        ) : (
